...
const rootPlannerProfileTopology: string | undefined = getPlannerProfile()?.topology;
const rootPlannerProfileLaneSource: string | undefined = getPlannerProfile()?.laneSource;
const rootPlannerProfileAdaptive: boolean | undefined = getPlannerProfile()?.adaptive;
const rootPlannerProfileExecutionModel: string | undefined = getPlannerProfile()?.executionModel;
const coordinationPlannerProfileExecutionModel: string | undefined = getPlannerProfile("coordination-local")?.executionModel;
const coordinationPlannerProfileLaneModel: string | undefined = getPlannerProfile("coordination-local")?.laneModel;
...
const rootSwarmPlanExecutionShape: "solo-lane" | "serial-handoff" | "parallel-handoff" = planSwarm("typed root swarm", { profileId: "bounded-local" }).swarm.executionShape;
const rootSwarmPlanWaveCount: number = planSwarm("typed root swarm", { profileId: "bounded-local" }).swarm.waveCount;
const rootSwarmPlanParallelizable: boolean | undefined = planSwarm("typed root swarm", { profileId: "bounded-local" }).swarm.waves[0]?.parallelizable;
...
const rootTaskPlanPlannerTopology: string = planTask("typed smoke", { profileId: "bounded-local" }).planner.topology;
const rootTaskPlanPlannerLaneSource: string = planTask("typed smoke", { profileId: "bounded-local" }).planner.laneSource;
...
const apiPlannerProfileExecutionModel: string | undefined = getApiPlannerProfile()?.executionModel;
...
const swarmHistoryType: string | undefined = swarm.history?.[0]?.type;
...
const swarmHasHistory: boolean | undefined = getSwarmView(swarm.id)?.metadata.hasHistory;
const swarmHistoryEntries: number | undefined = getSwarmView(swarm.id)?.metadata.historyEntries;
...
const subpathPlannerProfileTopology: string | undefined = getPlannerProfileSubpath()?.topology;
const subpathPlannerProfileExecutionModel: string | undefined = getPlannerProfileSubpath()?.executionModel;
const coordinationSubpathPlannerProfileExecutionModel: string | undefined = getPlannerProfileSubpath("coordination-local")?.executionModel;
const coordinationSubpathPlannerProfileLaneModel: string | undefined = getPlannerProfileSubpath("coordination-local")?.laneModel;
...
const queuedPlanExecutionShape: "solo-lane" | "serial-handoff" | "parallel-handoff" = queueTasksFromPlan("typed queued plan", (tasks) => tasks as ReturnType<typeof addTask>[], { profileId: "bounded-local" }).orchestration.executionShape;

...
`status` and the MCP `runtime_status` tool summarize the current local runtime: shipped tool/agent/skill counts, persisted task/swarm/memory counts, queue status distribution, and a top-level highlight rollup of the most important orchestration semantics. `capabilities` and `runtime_capabilities` provide the fuller product-facing inventory of what this Codex-only runtime actually supports today, including semantic highlights such as launch-first queue orchestration and compact launch context in summary surfaces, plus preferred entrypoints for consumers that need the best CLI/MCP starting surface without scanning the full command list.

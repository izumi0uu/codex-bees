...
`status` and the MCP `runtime_status` tool summarize the current local runtime: shipped tool/agent/skill counts, persisted task/swarm/memory counts, queue status distribution, and a top-level highlight rollup of the most important orchestration semantics. `capabilities` and `runtime_capabilities` provide the fuller product-facing inventory of what this Codex-only runtime actually supports today, including semantic highlights such as launch-first queue orchestration and compact launch context in summary surfaces, preferred entrypoints for consumers that need the best CLI/MCP starting surface, and focused use cases that explain when each high-value capability should be selected.
...
`runtime:queue-pack` / `runtime_queue_pack` provide the queue-oriented package: leader startup launch context, leader queue, dashboard queue context, and current focus combined into one queue-control payload with a recommended next surface. When parallel startup is ready it prefers `leader:assignment-launch-plan`; when a single ready launch exists it prefers `leader:assignment-dispatch-bundle`; otherwise it falls back to the raw leader queue review surface. Pass `--workers` on CLI or `workerIds` over MCP when the nested launch context should emit concrete worker-targeted startup commands instead of placeholder worker ids.

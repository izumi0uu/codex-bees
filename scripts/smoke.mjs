...
const installedCommandsExample = spawnSync(
  ...
  [
    "--input-type=module",
    "-e",
    `${documentedCommandsExampleScript}\nconsole.log(JSON.stringify({ ok: catalog.kind === "command_catalog_view" && catalog.commands.some((entry) => entry.command === "mcp") && initEntry?.command === "init" && initHelpView?.kind === "command_help_view" && initHelpView?.matchedCommand === "init" && initHelpView?.text.includes("codex-bees init") && Array.isArray(initEntry?.options) && initOptions.some((option) => option.option === "--preview") && previewOption?.option === "--preview" && initOptionHelpView?.kind === "init_help_view" && initOptionHelpView?.matchedOption === "--preview" && initOptionHelpView?.text.includes("codex-bees init") && initHelp.includes("codex-bees init") && help.includes("codex-bees run") && help.includes("codex-bees mcp") }));`
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"
...
const installedMcpImport = spawnSync(
  ...
  [
    "-e",
    'import("codex-bees/mcp").then((m) => { const listed = m.handleMcpRequest({ jsonrpc: "2.0", id: 1, method: "tools/list" }); const contract = m.callMcpTool("runtime_contract"); const serialized = m.serializeMcpMessage({ jsonrpc: "2.0", id: 2, method: "initialize" }); const runtimeContractTool = m.getMcpToolEntry("runtime_contract"); const runtimeContractToolView = m.getMcpToolView("runtime_contract"); const missingToolView = m.getMcpToolView("missing_tool"); const mcpOptions = m.getMcpCommandCatalog(); const mcpCatalog = m.getMcpCommandCatalogView(); const mcpHelp = m.renderMcpHelpText(); const toolsOption = m.getMcpCommandCatalogEntry("--tools"); const helpView = m.getMcpHelpView("--tools"); const fallbackHelpView = m.getMcpHelpView("--missing"); console.log(JSON.stringify({ ok: Array.isArray(m.listMcpTools()) && m.listMcpTools().some((tool) => tool.name === "runtime_contract") && runtimeContractTool?.name === "runtime_contract" && runtimeContractToolView?.kind === "mcp_tool_view" && runtimeContractToolView?.matchedTool === "runtime_contract" && runtimeContractToolView?.tool?.name === "runtime_contract" && missingToolView?.recommendedReason === "mcp_tool_missing" && listed.result?.tools?.some((tool) => tool.name === "runtime_contract") && Array.isArray(contract.content) && contract.content[0]?.type === "text" && serialized.endsWith("\\n") && Array.isArray(mcpOptions) && mcpOptions.some((option) => option.option === "--capabilities") && toolsOption?.option === "--tools" && helpView.kind === "mcp_help_view" && helpView.matchedOption === "--tools" && helpView.text.includes("codex-bees mcp --tools") && fallbackHelpView.recommendedReason === "mcp_help_fallback_loaded" && mcpCatalog.kind === "mcp_command_catalog_view" && mcpCatalog.counts.totalOptions >= 5 && mcpCatalog.options.some((option) => option.option === "--capabilities") && mcpHelp.includes("codex-bees mcp --tools") && mcpHelp.includes("codex-bees mcp --capabilities"), keys: Object.keys(m).sort() })); })'
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"
...
const installedMcpExample = spawnSync(
  ...
  [
    "--input-type=module",
    "-e",
    `${documentedMcpExampleScript}\nconsole.log(JSON.stringify({ ok: Array.isArray(tools) && tools.some((tool) => tool.name === "runtime_contract") && runtimeContractTool?.name === "runtime_contract" && runtimeContractToolView?.kind === "mcp_tool_view" && runtimeContractToolView?.matchedTool === "runtime_contract" && Array.isArray(options) && options.some((option) => option.option === "--capabilities") && toolsOption?.option === "--tools" && helpView?.kind === "mcp_help_view" && helpView?.matchedOption === "--tools" && listed.result?.tools?.some((tool) => tool.name === "runtime_contract") && Array.isArray(contract.content) && contract.content[0]?.type === "text" }));`
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"
...
const installedCommandsImport = spawnSync(
  ...
  [
    "-e",
    'import("codex-bees/commands").then((m) => { const mcp = m.getCommandCatalogEntry("mcp"); const init = m.getCommandCatalogEntry("init"); const initHelpView = m.getCommandHelpView("init"); const missingHelpView = m.getCommandHelpView("missing"); const previewOption = m.getInitCommandCatalogEntry("--preview"); const initOptionHelpView = m.getInitHelpView("--preview"); const fallbackInitOptionHelpView = m.getInitHelpView("--missing"); console.log(JSON.stringify({ ok: m.getCommandCatalogView().kind === "command_catalog_view" && m.getCommandCatalogView().counts.totalCommands > 10 && m.renderHelpText().includes("codex-bees run") && m.renderHelpText().includes("codex-bees metadata") && typeof m.getCommandCatalogEntry === "function" && m.getCommandCatalogEntry("init")?.command === "init" && typeof m.getCommandHelpView === "function" && initHelpView.kind === "command_help_view" && initHelpView.matchedCommand === "init" && initHelpView.text.includes("codex-bees init") && missingHelpView.recommendedReason === "command_help_fallback_loaded" && missingHelpView.text.includes("codex-bees run") && typeof m.getInitCommandCatalogEntry === "function" && previewOption?.option === "--preview" && typeof m.getInitHelpView === "function" && initOptionHelpView.kind === "init_help_view" && initOptionHelpView.matchedOption === "--preview" && initOptionHelpView.text.includes("codex-bees init") && fallbackInitOptionHelpView.recommendedReason === "init_help_fallback_loaded" && Array.isArray(mcp?.options) && mcp.options.some((option) => option.option === "--capabilities") && mcp.options.some((option) => option.option === "--tools") && typeof m.getInitCommandCatalog === "function" && Array.isArray(m.getInitCommandCatalog()) && m.getInitCommandCatalog().some((option) => option.option === "--preview") && Array.isArray(init?.options) && init.options.some((option) => option.option === "--dir <path>") && typeof m.renderCommandHelpText === "function" && m.renderCommandHelpText("init").includes("codex-bees init") && m.renderCommandHelpText("missing").includes("codex-bees run") })); })'
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"

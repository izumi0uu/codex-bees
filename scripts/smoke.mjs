...
const installedCommandsExample = spawnSync(
  ...
  [
    "--input-type=module",
    "-e",
    `${documentedCommandsExampleScript}\nconsole.log(JSON.stringify({ ok: catalog.kind === "command_catalog_view" && catalog.commands.some((entry) => entry.command === "mcp") && initEntry?.command === "init" && Array.isArray(initEntry?.options) && initOptions.some((option) => option.option === "--preview") && initHelp.includes("codex-bees init") && help.includes("codex-bees run") && help.includes("codex-bees mcp") }));`
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"
...
const installedCommandsImport = spawnSync(
  ...
  [
    "-e",
    'import("codex-bees/commands").then((m) => { const mcp = m.getCommandCatalogEntry("mcp"); const init = m.getCommandCatalogEntry("init"); console.log(JSON.stringify({ ok: m.getCommandCatalogView().kind === "command_catalog_view" && m.getCommandCatalogView().counts.totalCommands > 10 && m.renderHelpText().includes("codex-bees run") && m.renderHelpText().includes("codex-bees metadata") && typeof m.getCommandCatalogEntry === "function" && m.getCommandCatalogEntry("init")?.command === "init" && Array.isArray(mcp?.options) && mcp.options.some((option) => option.option === "--capabilities") && mcp.options.some((option) => option.option === "--tools") && typeof m.getInitCommandCatalog === "function" && Array.isArray(m.getInitCommandCatalog()) && m.getInitCommandCatalog().some((option) => option.option === "--preview") && Array.isArray(init?.options) && init.options.some((option) => option.option === "--dir <path>") && typeof m.renderCommandHelpText === "function" && m.renderCommandHelpText("init").includes("codex-bees init") && m.renderCommandHelpText("missing").includes("codex-bees run") })); })'
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"

...
const runtimeReadyMcpPayload = JSON.parse(JSON.parse(runtimeContractMcpLines[11]).result.content[0].text);
const runtimeContractMcpPayload = JSON.parse(JSON.parse(runtimeContractMcpLines[12]).result.content[0].text);
if (
  ...
  console.error("[smoke:metadata-doctor-commands-entry-help-init-option-help-mcp-options-option-ready-contract-mcp] expected MCP metadata, doctor, command catalog, command entry, command help, init options, init option, init help, MCP options, MCP option, readiness, and contract views");

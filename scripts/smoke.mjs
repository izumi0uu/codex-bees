...
const installedMcpExample = spawnSync(
  ...
  [
    "--input-type=module",
    "-e",
    `${documentedMcpExampleScript}\nconsole.log(JSON.stringify({ ok: Array.isArray(tools) && tools.some((tool) => tool.name === "runtime_contract") && runtimeContractTool?.name === "runtime_contract" && Array.isArray(options) && options.some((option) => option.option === "--capabilities") && toolsOption?.option === "--tools" && helpView?.kind === "mcp_help_view" && helpView?.matchedOption === "--tools" && listed.result?.tools?.some((tool) => tool.name === "runtime_contract") && Array.isArray(contract.content) && contract.content[0]?.type === "text" }));`
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"

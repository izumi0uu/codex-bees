...
const installedCommandsExample = spawnSync(
  ...
  [
    "--input-type=module",
    "-e",
    `${documentedCommandsExampleScript}\nconsole.log(JSON.stringify({ ok: catalog.kind === "command_catalog_view" && catalog.commands.some((entry) => entry.command === "mcp") && initEntry?.command === "init" && initEntryView?.kind === "command_catalog_entry_view" && initEntryView?.matchedCommand === "init" && initHelpView?.kind === "command_help_view" && initHelpView?.matchedCommand === "init" && initHelpView?.text.includes("codex-bees init") && Array.isArray(initEntry?.options) && Array.isArray(initOptions) && initOptions.some((option) => option.option === "--preview") && initOptionsView?.kind === "init_command_catalog_view" && initOptionsView?.counts?.totalOptions >= 5 && initOptionsView?.options?.some((option) => option.option === "--preview") && previewOption?.option === "--preview" && previewOptionView?.kind === "init_command_option_view" && previewOptionView?.matchedOption === "--preview" && initOptionHelpView?.kind === "init_help_view" && initOptionHelpView?.matchedOption === "--preview" && initOptionHelpView?.text.includes("codex-bees init") && initHelp.includes("codex-bees init") && help.includes("codex-bees run") && help.includes("codex-bees mcp") }));`
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"
...
const cliCommandHelpView = JSON.parse(
  run("command-help-verify", ["./src/index.js", "command:help", "--name", "init"]).stdout
).help;
if (
  cliCommandHelpView.kind !== "command_help_view" ||
  cliCommandHelpView.recommendedReason !== "command_help_loaded" ||
  cliCommandHelpView.matchedCommand !== "init" ||
...
const runtimeReadyMcpPayload = JSON.parse(JSON.parse(runtimeContractMcpLines[12]).result.content[0].text);
const runtimeContractMcpPayload = JSON.parse(JSON.parse(runtimeContractMcpLines[13]).result.content[0].text);
if (
  ...
  console.error("[smoke:metadata-doctor-commands-entry-help-init-option-help-mcp-options-option-help-ready-contract-mcp] expected MCP metadata, doctor, command catalog, command entry, command help, init options, init option, init help, MCP options, MCP option, MCP help, readiness, and contract views");

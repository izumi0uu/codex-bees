...
const installedCommandsExample = spawnSync(
  ...
  [
    "--input-type=module",
    "-e",
    `${documentedCommandsExampleScript}\nconsole.log(JSON.stringify({ ok: catalog.kind === "command_catalog_view" && catalog.commands.some((entry) => entry.command === "mcp") && initEntry?.command === "init" && initHelpView?.kind === "command_help_view" && initHelpView?.matchedCommand === "init" && initHelpView?.text.includes("codex-bees init") && Array.isArray(initEntry?.options) && initOptions.some((option) => option.option === "--preview") && initHelp.includes("codex-bees init") && help.includes("codex-bees run") && help.includes("codex-bees mcp") }));`
  ],
  {
    cwd: packedInstallAppDir,
    encoding: "utf8"

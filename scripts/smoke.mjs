...
const runtimeReadyMcpPayload = JSON.parse(JSON.parse(runtimeContractMcpLines[7]).result.content[0].text);
const runtimeContractMcpPayload = JSON.parse(JSON.parse(runtimeContractMcpLines[8]).result.content[0].text);
if (
  ...
  console.error("[smoke:metadata-doctor-commands-entry-help-init-ready-contract-mcp] expected MCP metadata, doctor, command catalog, command entry, command help, init options, readiness, and contract views");
